...
核心库还支持背景扣除 `subtract_background`，但 v0.1.1 的批量 GUI 尚未提供空白文件选择控件。
...
说明: v0.1.1 中“加载项目”菜单项也绑定了 `Ctrl+O`，如果快捷键行为与预期不一致，请直接点击“项目”页中的加载按钮。
...
Echem Analyzer v0.1.1 中 Rs/Rct 是快速估算，不是等效电路拟合。专用 EIS 软件通常使用电路模型、非线性拟合和误差评估，因此结果可能不同。

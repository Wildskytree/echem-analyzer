...
- CHI Instruments 文本/CSV 文件: `.txt`, `.csv`
...
- CHI Instruments 导出的 `.csv` 文件现在可直接导入，之前只支持 `.txt`。
...
其中 `I` 使用归一化前的电流，单位 A；`Rs` 单位 Ω。若项目中已有 EIS 数据，可点击"从 EIS 获取 Rs"，在弹窗中选择使用哪条 EIS 数据来估算并填入 Rs。
...
LSV 标签页底部提供"导出曲线数据"按钮，可将当前处理后（含 RHE 换算、iR 补偿、归一化、平滑）的曲线数据一键导出为 CSV 文件：

- 导出的数据包含处理后的电位和电流列。
- 文件名默认使用原始文件名加 `_processed` 后缀。
- CSV 使用 UTF-8 with BOM 编码，便于 Windows Excel 直接打开中文字符。
- 若当前未执行分析或参数为空，按钮会提示先执行分析。

> **提示**: 在"多曲线对比"视图中同样提供导出按钮，可导出所有已勾选曲线的合并 CSV，方便 Origin 或其他软件进一步绘图分析。
...
CV 标签页底部提供“导出单圈”按钮，可将多圈 CV 中指定圈数的数据单独保存。

操作步骤:

1. 在“当前文件”中选择一条 CV。
2. 使用按钮旁边的圈数选择器设置圈数。该选择器为 `QSpinBox`，范围为 1-99。
3. 点击“导出单圈”。
4. 在保存对话框中确认路径。默认文件名为 `cv_cycle_N.txt`，其中 `N` 为选择的圈数。
...
CV 标签页提供“按电压分圈”按钮，用于按电位窗口辅助拆分多圈数据。

操作步骤:

1. 点击“按电压分圈”。
2. 在弹窗中输入最低电位和最高电位。
3. 软件会自动检测电位转折点，将多圈 CV 数据拆分为单圈。
4. 弹窗会显示每圈数据点数。
5. 选择导出某一圈或全部圈数。
...
核心库还支持背景扣除 `subtract_background`，但 v0.1.2 的批量 GUI 尚未提供空白文件选择控件。
...
勾选"导出数据到 Origin (.txt)"，会输出制表符分隔文本。
...
v0.1.2 新增多曲线对比功能，支持在同一张图上叠加显示多条曲线，方便直观比较不同条件（如不同转速、不同扫速、不同电位窗口）下的电化学行为。

#### LSV 多曲线对比

LSV 标签页的"多曲线叠加对比"区域支持以下高级特性：

- **独立处理参数**: 每条曲线可分别设置 RHE 换算（参比电极、pH）、iR 补偿（Rs、补偿比例）、电流归一化（模式、面积、负载量）和 Savitzky-Golay 平滑参数。参数修改后无需重新导入。
- **勾选显示/隐藏**: 每条曲线前有复选框，勾选时在图中显示该曲线，取消勾选时隐藏，用于快速筛选感兴趣的数据。
- **独立执行分析**: 每条曲线可单独执行分析，或者选中后批量分析。
- **导出合并数据**: 点击"导出曲线数据"可将所有勾选且显示的曲线处理后数据合并导出为一个 CSV 文件，每列包含曲线名和对应的电位/电流数据。
- **图例自动更新**: 图例随勾选自动更新，仅显示当前可见的曲线。

操作步骤:

1. 在"当前文件"下拉框中选择一条 LSV 数据。
2. 在参数区设置好处理参数。
3. 点击"添加曲线"将该数据加入对比列表。
4. 重复步骤 1-3 添加多条曲线。
5. 在列表中勾选需要显示的曲线，取消勾选不想显示的曲线。
6. 点击"更新对比图"或"执行分析"刷新图表。
7. 如需修改某条曲线的处理参数，双击该曲线行或点击"编辑参数"按钮。
...
- 在列表中选择多条 EIS 数据。
- 勾选控制每条曲线的显示/隐藏。
- Nyquist 图和 Bode 图均同步更新。
- 适合比较不同电位或不同条件下的阻抗谱变化。
...
说明: v0.1.2 中“加载项目”菜单项也绑定了 `Ctrl+O`，如果快捷键行为与预期不一致，请直接点击“项目”页中的加载按钮。
...
- 文件不是 `.txt` 或 `.csv`。
- CHI Instruments 导出的 `.csv` 文件已支持直接导入；若导入失败，请确认文件仍为文本 CSV，而不是 Excel 或其他二进制格式。
...
Echem Analyzer v0.1.2 中 Rs/Rct 是快速估算，不是等效电路拟合。专用 EIS 软件通常使用电路模型、非线性拟合和误差评估，因此结果可能不同。
